...
- Builds the wheel in a dedicated `build` job, then runs the test matrix against the installed wheel (not `uv run`). See [.github/workflows/test-matrix.yml](.github/workflows/test-matrix.yml). Python 3.12–3.14 × Django 5.2–6.1 with matrix exclusions.
