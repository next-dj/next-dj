...
| --- | --- |
| [next/apps/](next/apps/) | Django `AppConfig`s that wire components, templates, static files, and autoreload into Django startup. |
...
| [tests/benchmarks/](tests/benchmarks/) | Micro-benchmarks. **Opt-in**; excluded from the default run via `--ignore=tests/benchmarks`. |
...
| `uv run pytest tests/benchmarks --benchmark-only --no-cov` | Opt-in benchmark run. Numbers are only comparable on the same machine. |
...
| `make type-check` | Mypy on `next/`. Strict config (`disallow_untyped_defs`, Django plugin). |
| `make lint-js` | ESLint on TypeScript. |
| `make format-js` | Prettier write. |
| `make format-js-check` | Prettier check (CI). |
...
Use [examples/file-routing/tests/conftest.py](examples/file-routing/tests/conftest.py) as the template for Django settings, `INSTALLED_APPS`, and the `client` fixture.
