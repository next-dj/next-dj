...
An exception that belongs to one subsystem lives in that subsystem's ``errors`` module instead, such as ``next.urls.errors``, ``next.partial.errors``, or ``next.pages.errors``, which holds ``PageModuleImportError`` and ``PageContextShapeError``.
Those two are not configuration errors, so they subclass ``Exception`` and ``TypeError`` rather than the Django one, and :doc:`pages` documents both.
