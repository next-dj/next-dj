...
``resolve_setting_class`` serves the other shape, a key holding a single dotted path rather than a list of backend entries, and is what reads ``URL_RESOLVER``, ``DEPENDENCY_RESOLVER``, and ``COMPONENT_TEMPLATE_LOADER``.
Its ``scope`` argument reads the key inside a mapping setting instead of at the top level, which is how ``RENDERER`` inside ``METADATA`` is read, and a key missing from that mapping reads as its default.
