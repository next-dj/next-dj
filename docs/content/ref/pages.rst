...
It also exposes the page metadata surface, the folded ``Metadata`` value, the ``MetadataDict`` and ``SiteMetadataDict`` input shapes, the ``MetadataRenderer`` contract with its ``HtmlMetadataRenderer``, and the four ``PageMetadata*`` errors, see `Metadata`_ below.
...
Ten ``Page`` methods carry no leading underscore because other framework areas call them, not because application code should.
``composed_template_for``, ``build_render_context``, ``render_with_static_assets``, ``authorization_outcome``, ``has_template``, ``zone_bindings``, ``create_url_pattern``, ``render``, ``clear_template_caches``, and ``templated_title`` serve ``next.forms``, ``next.partial``, ``next.urls``, and ``next.testing``.
``next.forms`` and ``next.partial`` read the first six, ``next.urls`` builds every page pattern through ``create_url_pattern``, ``next.testing`` calls ``render`` and ``clear_template_caches`` from its rendering and isolation helpers, and ``next.partial`` reads ``templated_title`` for the ``meta`` patch verb.
...
``next.pages.metadata`` folds the settings tier and every ``metadata`` dict or ``@page.metadata`` callable along the ancestor chain of a page into one ``Metadata`` value, which ``{% metadata %}`` renders through the default ``HtmlMetadataRenderer``.
:doc:`/content/topics/seo/metadata` covers the declaration forms and the merge order.
``Page.metadata`` is the decorator, ``Page.static_metadata`` folds the settings tier and the dicts of a page without a request, ``Page.templated_title`` applies the chain template to one title the way that page would render it, and ``Page.resolve_metadata`` runs the whole chain, callables included, for a request built by the caller.
...
The four metadata errors name the source that misbehaved.
...
They run through ``uv run python manage.py check``, except ``check_page_module_imports``, which is a deployment check and runs under ``manage.py check --deploy``, and the four ``check_seo_*`` audits, which run under ``manage.py check --deploy --tag seo``.
