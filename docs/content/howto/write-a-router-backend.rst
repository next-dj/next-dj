...
The names the walk refuses to enter are the folder ``components_folder_name`` returns plus the ``DIRS`` entries of ``PAGE_BACKENDS`` that name no existing directory, which is the same skip set the file router builds from those settings.

Both are read on the ``runserver``, ``collectstatic``, and staticfiles paths, so an override that reaches a database or a network is called where an outage must not stop the process.
