...
Injecting context by name: DContext and DGlobalContext
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

You can **explicitly** request values by name using two markers:

- **``DContext["key"]``** — Injects the value of a **context variable** already
...
the value is injected without using ``DContext``.
...
- **layouts/**: Context inheritance, **DContext** (parent context by name), and
  **DGlobalContext** (layout-level global dependency) — see ``layouts/pages/page.py``
