...
The next three bind the ``next.ports`` slots, the composition the request path, the watcher, and the checks all depend on, and they run early for the same reason the resolver setting does.
