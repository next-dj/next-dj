...
def _by_url_slug(model_cls: type[Model], context: ResolutionContext) -> Model:
    """Fetch the row named by the URL `slug`, or raise `Http404`.

    Both the plain `resolve` path and the compiled filler land here, so the
    lookup and its failure mode are written once for the two of them.
    """
...
class LinkProvider(RegisteredParameterProvider):
    ...
    def static_can_handle(self, param: inspect.Parameter) -> bool:
        """Claim every `DLink[...]` parameter and rule out all the others for good.
