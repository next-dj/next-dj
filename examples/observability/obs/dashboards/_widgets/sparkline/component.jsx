 ...
 *
 * Reads `window.Next.context.totals_chart` from the matching `@component.context`
 * callable. The wrapper override shapes the payload as `{v, data}`, so the bars live
 * under `.data.bars`, one rounded bar per source scaled to the largest value.
 *
 * Babel-standalone transforms and runs this file after DOMContentLoaded, so React
 * and ReactDOM are in place when `Next.partial.onMount` replays over the document.
 */
