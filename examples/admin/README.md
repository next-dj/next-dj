...
[`shadcn_admin/surfaces/[str:app_label]/[str:model_name]/page.py`](shadcn_admin/surfaces/%5Bstr%3Aapp_label%5D/%5Bstr%3Amodel_name%5D/page.py) asks `ModelAdmin.get_changelist_instance(request)` and packs the result for the template. The `changelist_state` callable is a thin orchestrator that delegates to `_columns`, `_rows`, `_pagination`, `_filters`, and `_actions` helpers in the same file, so the response shape fits on one screen. The synthetic `action_checkbox` column that Django injects into `cl.list_display` is filtered out because we render selection ourselves through the `selectable=` prop on `data_table`. The bulk-action list comes from `ModelAdmin.get_action_choices(request, default_choices=[])` rather than `get_actions`, because that method already interpolates the model's `verbose_name` and `verbose_name_plural` — `delete_selected`'s `%(verbose_name_plural)s` placeholder arrives as `Delete selected books` — and its `(name, description)` pairs read the same on every supported Django version, while `get_actions` answers a `(callable, name, description)` tuple on 5.2 and an `Action` dataclass from 6.0 on.
...
**Every id is namespaced, because two pages share one document.** Each row builds the same unprefixed form, so without help they would all render `id_title` and steal the parent form's label. The rows go through `auto_id` (`id_chapter_<pk>_title`, `id_chapter_add_title`) rather than a `prefix=`, because `auto_id` renames only the ids while the wire keeps posting the plain field names the handlers read. The main form gets the same treatment from `AdminFormSpec.auto_id` (`id_book_title`, `id_chapter_title`): once `layer_open` drops the chapter change view on top of the book change view, both editors live in one DOM, and a bare `id_%s` would put `id_title` in the document twice, so `<label for="id_title">` inside the modal would focus the book behind it.

**Uniqueness needs the parent key in scope.** `Chapter` declares `unique_together = [("book", "number")]`, but the foreign key back to the book is never a field on a keyed row form. Django's `ModelForm` excludes fields the form does not carry from model validation, so every unique check naming `book` is skipped and a duplicate chapter number reaches the database as an `IntegrityError` — a 500, not a form error. An inline formset avoids this by adding the parent link as a form field. A standalone row form has no formset to do that, so `_row_form_class` overrides `validate_unique()` to subtract the foreign key from the exclusion set Django computed:

```python
class InlineRowForm(base):
...
The row's instance already carries the parent (an existing child through `_inline_pk`, a new one through `blank()`), so the pair check runs against the right book and a duplicate number comes back as a non-field error on that row. The same class binds the render and the dispatch, so the error shows up whether the submit was a partial morph or a full-page re-render.
