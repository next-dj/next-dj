...
class TestRouting:
    ...
    def test_landing_renders(self, client) -> None:
    ...
    def test_landing_without_show_falls_back_to_the_default(self, client) -> None:
    ...
    def test_landing_show_query_param(self, client, show, expected_count) -> None:
        """Honour `?show=N` on the landing through `DQuery[int]`."""
        body = client.get(f"/?show={show}").content.decode()
        assert body.count("data-product-card") == expected_count

    def test_landing_reuses_product_card_css(self, client) -> None:
        """Render the shared `product_card.css` once on the landing too."""
        body = client.get("/").content.decode()
        assert body.count("data-product-card") == 3
        assert body.count("product_card.css") == 1

    def test_listing_renders(self, client) -> None:
    ...
    def test_product_card_css_dedup(self, client) -> None:
    ...
    def test_catalog_layout_css_absent_on_landing(self, client) -> None:
...
class TestFilters:
    ...
    def test_filter_by_brand_keeps_only_listed_brands(
        self, client, query, expected_brands
    ) -> None:
    ...
    def test_search_query_matches_product_name(self, client) -> None:
        """Match products whose name contains the search term."""
        r = client.get("/catalog/?q=iPhone")
        assert r.status_code == 200
        assert "iPhone 15" in r.content.decode()

    def test_pagination_returns_distinct_pages(self, client) -> None:
...
class TestActiveFilterChip:
    ...
    def test_chip_renders_with_class_and_data_attribute(self, client) -> None:
    ...
    def test_no_chip_on_default_sort(self, client) -> None:
...
class TestInheritContext:
    ...
    def test_product_detail_uses_inherited_category(self, client) -> None:
...
class TestCacheHit:
    ...
    def test_two_identical_requests_share_one_cache_key(self, client) -> None:
        """Store one cache entry when the same filters arrive twice in a row."""
        client.get("/catalog/?brand=Acme")
        client.get("/catalog/?brand=Acme")
        assert len(_cache_keys()) == 1

    def test_distinct_filters_produce_distinct_keys(self, client) -> None:
...
class TestResultsZone:
    ...
    def test_listing_wraps_results_in_a_named_zone(self, client) -> None:
        body = client.get("/catalog/").content.decode()
        assert 'data-next-zone="catalog-results"' in body
        assert '<ul data-next-zone="catalog-results"' in body

    def test_filter_form_carries_auto_submit_attributes(self, client) -> None:
        body = client.get("/catalog/").content.decode()
    ...
    def test_zone_request_morphs_only_the_results(self, client) -> None:
    ...
    def test_zone_request_narrows_to_the_search_term(self, client) -> None:
    ...
    def test_zone_request_stamps_the_partial_vary_headers(self, client) -> None:
        response = client.get_zones("/catalog/", "catalog-results")
        vary = response["Vary"]
...
class TestInfiniteScrollAppend:
    ...
    def test_first_page_renders_a_sentinel_link(self, client) -> None:
        body = client.get("/catalog/").content.decode()
    ...
    def test_sentinel_lives_outside_the_results_zone(self, client) -> None:
        body = client.get("/catalog/").content.decode()
    ...
    def test_append_grows_results_and_replaces_the_sentinel(self, client) -> None:
    ...
    def test_append_to_results_carries_rows_only_not_the_sentinel(self, client) -> None:
        envelope = envelope_of(
            client.get_zones(
                "/catalog/?page=2",
                "catalog-results,catalog-more",
    ...
    def test_appended_rows_do_not_overlap_the_first_page(self, client) -> None:
    ...
    def test_last_page_drops_the_sentinel(self, client) -> None:
        body = client.get("/catalog/?page=999").content.decode()
        assert "results-sentinel" not in body

    def test_last_page_append_empties_the_more_zone(self, client) -> None:
        envelope = envelope_of(
            client.get_zones(
                "/catalog/?page=999",
                "catalog-results,catalog-more",
    ...
    def test_changing_the_query_re_morphs_both_zones(self, client) -> None:
        envelope = envelope_of(
...
class TestPresetFilterPushUrl:
    ...
    def test_listing_renders_the_preset_bar(self, client) -> None:
        body = client.get("/catalog/").content.decode()
    ...
    def test_preset_bar_targets_the_full_listing_zone_set(self, client) -> None:
        body = client.get("/catalog/").content.decode()
        preset_form = body.split("data-preset-bar", 1)[0].rsplit("<form", 1)[1]
        assert f'data-next-target="{LISTING_TARGET}"' in preset_form

    def test_preset_apply_refreshes_the_count_and_the_chips(self, client) -> None:
        envelope = envelope_of(
            client.post_action(
                "preset_filter_form",
                {"preset": "in_stock"},
    ...
    def test_url_op_pushes_the_canonical_same_site_href(self, client) -> None:
        envelope = envelope_of(
            client.post_action(
                "preset_filter_form",
                {"preset": "cheapest"},
    ...
    def test_in_stock_preset_morph_drops_out_of_stock_products(self, client) -> None:
        envelope = envelope_of(
            client.post_action(
                "preset_filter_form",
                {"preset": "in_stock"},
    ...
    def test_morphed_sentinel_carries_the_preset_querystring(self, client) -> None:
        envelope = envelope_of(
            client.post_action(
                "preset_filter_form",
                {"preset": "cheapest"},
    ...
    def test_non_partial_apply_falls_back_to_a_redirect(self, client) -> None:
...
class TestFilterZoneScope:
    ...
    def test_listing_declares_every_targeted_zone(self, client) -> None:
        body = client.get("/catalog/").content.decode()
        for zone in LISTING_ZONES:
            assert f'data-next-zone="{zone}"' in body

    def test_category_form_drops_the_append_sentinel_zone(self, client) -> None:
        body = client.get("/catalog/electronics/").content.decode()
    ...
    def test_product_detail_filter_form_makes_a_plain_get(self, client) -> None:
        body = client.get("/catalog/electronics/iphone-15/").content.decode()
        form_tag = _filter_form_tag(body)
    ...
    def test_category_zone_request_morphs_the_declared_set(self, client) -> None:
        envelope = envelope_of(
            client.get_zones("/catalog/electronics/?q=iPhone", CATEGORY_TARGET)
        )
...
class TestLiveFilterMorphsTheWholeListing:
    ...
    def test_count_zone_follows_the_search_term(self, client) -> None:
        envelope = envelope_of(client.get_zones("/catalog/?q=iPhone", LISTING_TARGET))
        assert "1 products" in envelope.html_for_zone("catalog-count")

    def test_pager_zone_collapses_to_a_single_page(self, client) -> None:
        envelope = envelope_of(client.get_zones("/catalog/?q=iPhone", LISTING_TARGET))
        pager = envelope.html_for_zone("catalog-pager")
        assert "Page 1 of 1" in pager
        assert "page=2" not in pager

    def test_chip_zone_gains_the_search_chip(self, client) -> None:
        envelope = envelope_of(client.get_zones("/catalog/?q=iPhone", LISTING_TARGET))
        chips = envelope.html_for_zone("catalog-chips")
        assert 'data-active-filter="q"' in chips
        assert "Search iPhone" in chips

    def test_chip_zone_renders_empty_without_filters(self, client) -> None:
        envelope = envelope_of(client.get_zones("/catalog/", LISTING_TARGET))
        chips = envelope.html_for_zone("catalog-chips")
