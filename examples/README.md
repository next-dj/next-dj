...
| [`markdown-blog/`](markdown-blog/) | Markdown posts, nested layouts, `@context(serialize=True)`, context processor, co-located `component.js`, page metadata with hreflang alternates plus a sitemap and a static `robots.txt` over `i18n_patterns` |
...
| [`search-catalog/`](search-catalog/) | `DQuery[T]`, faceted filters, three-level nested layouts, `inherit_context=True`, cached search, a canonical that drops the filter query and a `sitemap.py` over categories and products |
| [`wiki/`](wiki/) | `HybridRouterBackend`, `router_manager.reload()` on signal, `DArticle` DI provider, forms with live Markdown preview, a sitemap dated per article row beside a `robots.py` fencing the search |
| [`multi-tenant/`](multi-tenant/) | `TenantMiddleware` resolving `X-Tenant`, request-aware `TenantPrefixStaticBackend` rewriting co-located asset URLs per tenant on top of the staticfiles names it inherits resolution for, `STATIC_VERSION` stamping a deploy build id, project-shared `root_blocks/header` and `root_blocks/footer` via `COMPONENT_BACKENDS["DIRS"]`, per-tenant accent driven by a context processor |
...
| [`observability/`](observability/) | Every signal group wired through one receiver each, custom `ComponentsBackend` and `DedupStrategy`, `JsContextSerializer` swapped both globally and per-decorator on `live_stats`, React sparkline via CDN-loaded Babel-standalone, nested layout with filter form action, opt-in `ManifestStaticFilesStorage` profile where a named asset picks up a content hash and a vendor URL does not |
...
Tailwind is loaded via the Play CDN (`https://cdn.tailwindcss.com`) through the shared `page_head` component that every root layout calls. The same component renders the `<title>` and the rest of the head tags through `{% metadata %}`, so every example declares its site name and title template under `NEXT_FRAMEWORK["METADATA"]["DEFAULTS"]` and a page that wants a different tab title says so in its own `page.py`. The two Vite examples add an `npm install` plus a dev server or a build on top, documented in their own READMEs.
...
- `STATIC_VERSION` names the deploy stamp. Every asset URL carries it as a `v` parameter and every partial response carries it as the asset version, so shipping new assets asks open clients to reload instead of patching against stale JavaScript. The derived default reads `STATIC_VERSION` first and falls back to the hash of a staticfiles manifest, so one release tag serves both and nothing names a version twice. [`observability`](observability/) sets no tag at all, because its opt-in `ManifestStaticFilesStorage` profile is what the fallback reads. [`kanban`](kanban/) and [`live-polls`](live-polls/) let Vite hash their filenames, so they leave asset URLs untouched and pin the partial `VERSION` by itself through `next.conf.extend_default_backend`.
...
- A `sitemap.py` or a robots source at the top of a page root switches `/sitemap.xml` and `/robots.txt` on, both served through `include("next.urls")` with no URL of their own, or through `include("next.seo.urls")` at the host root when `next.urls` sits under a prefix or inside `i18n_patterns`. Static routes are listed on their own and dynamic ones through `@sitemap.items`, robots comes either as a static `robots.txt` served as is ([`markdown-blog`](markdown-blog/)) or as a `robots.py` declaring `Rule` groups ([`wiki`](wiki/), [`shortener`](shortener/)). Every example that publishes one sets `METADATA["DEFAULTS"]["base"]`, so the documents are absolute on the published origin rather than on the request host, and [`admin`](admin/) ships neither because a private site has nothing to list.
