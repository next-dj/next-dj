from __future__ import annotations

from typing import TYPE_CHECKING, Any

import pytest
from django.test import override_settings

from next.conf import NextFrameworkSettings, next_framework_settings, perform_import


if TYPE_CHECKING:
    from collections.abc import Generator


@pytest.fixture(autouse=True)
def _reload_next_framework_settings_after_test() -> Generator[None, None, None]:
    """Reload the global ``next_framework_settings`` after each test (teardown only)."""
    yield
    next_framework_settings.reload()


@pytest.fixture()
def fresh_next_framework_settings() -> NextFrameworkSettings:
    """Return a new ``NextFrameworkSettings`` (separate merge cache from globals)."""
    return NextFrameworkSettings()


class TestNextFrameworkSettingsDjangoIntegration:
    """Process-wide ``next_framework_settings`` with ``override_settings(NEXT_FRAMEWORK=...)``."""

    def test_empty_next_framework_uses_full_defaults(self) -> None:
        """Empty ``NEXT_FRAMEWORK`` keeps framework defaults for routers and backends."""
        with override_settings(NEXT_FRAMEWORK={}):
            next_framework_settings.reload()
            assert next_framework_settings.DEFAULT_PAGE_ROUTERS[0]["BACKEND"] == (
                "next.urls.FileRouterBackend"
            )

    def test_default_page_routers_replaces_default_list_entirely(self) -> None:
        """Providing ``DEFAULT_PAGE_ROUTERS`` replaces the default list."""
        custom = [
            {
                "BACKEND": "next.urls.FileRouterBackend",
                "PAGES_DIR": "custom_pages",
                "APP_DIRS": False,
                "OPTIONS": {},
            },
        ]
        with override_settings(NEXT_FRAMEWORK={"DEFAULT_PAGE_ROUTERS": custom}):
            next_framework_settings.reload()
            assert custom == next_framework_settings.DEFAULT_PAGE_ROUTERS
            assert (
                next_framework_settings.DEFAULT_PAGE_ROUTERS[0]["PAGES_DIR"]
                == "custom_pages"
            )

    def test_components_only_merge_leaves_routers_default(self) -> None:
        """Overriding only ``DEFAULT_COMPONENT_BACKENDS`` leaves page routers as defaults."""
        with override_settings(
            NEXT_FRAMEWORK={
                "DEFAULT_COMPONENT_BACKENDS": [
                    {
                        "BACKEND": "next.components.FileComponentsBackend",
                        "APP_DIRS": True,
                        "OPTIONS": {
                            "COMPONENTS_DIR": "_components",
                            "PAGES_DIR": "pages",
                            "COMPONENTS_DIRS": ["/tmp/x"],
                        },
                    },
                ],
            },
        ):
            next_framework_settings.reload()
            assert (
                next_framework_settings.DEFAULT_PAGE_ROUTERS[0]["PAGES_DIR"] == "pages"
            )
            assert next_framework_settings.DEFAULT_COMPONENT_BACKENDS[0]["OPTIONS"][
                "COMPONENTS_DIRS"
            ] == ["/tmp/x"]

    @pytest.mark.parametrize(
        "bad_attr",
        ["NOT_A_KEY", "FOO", "PAGES", ""],
        ids=["unknown_upper", "unknown_short", "legacy_pages", "empty_string"],
    )
    def test_unknown_attribute_raises(self, bad_attr: str) -> None:
        """Only top-level keys from DEFAULTS are valid attributes."""
        with pytest.raises(AttributeError, match="Invalid Next framework setting"):
            getattr(next_framework_settings, bad_attr)

    def test_next_framework_not_dict_falls_back_to_defaults(self) -> None:
        """Non-dict ``NEXT_FRAMEWORK`` is ignored and full defaults are used."""
        with override_settings(NEXT_FRAMEWORK=42):  # type: ignore[arg-type]
            next_framework_settings.reload()
            assert next_framework_settings.DEFAULT_PAGE_ROUTERS[0]["BACKEND"] == (
                "next.urls.FileRouterBackend"
            )

    def test_only_url_name_template_overridden(self) -> None:
        """Setting only ``URL_NAME_TEMPLATE`` keeps default ``DEFAULT_PAGE_ROUTERS``."""
        with override_settings(
            NEXT_FRAMEWORK={"URL_NAME_TEMPLATE": "view_{name}"},
        ):
            next_framework_settings.reload()
            assert next_framework_settings.URL_NAME_TEMPLATE == "view_{name}"
            assert (
                NextFrameworkSettings.DEFAULTS["DEFAULT_PAGE_ROUTERS"]
                == next_framework_settings.DEFAULT_PAGE_ROUTERS
            )

    def test_invalid_default_page_routers_type_keeps_default(self) -> None:
        """Non-list ``DEFAULT_PAGE_ROUTERS`` is ignored; merged value stays default."""
        with override_settings(NEXT_FRAMEWORK={"DEFAULT_PAGE_ROUTERS": "invalid"}):  # type: ignore[dict-item]
            next_framework_settings.reload()
            assert (
                NextFrameworkSettings.DEFAULTS["DEFAULT_PAGE_ROUTERS"]
                == next_framework_settings.DEFAULT_PAGE_ROUTERS
            )

    def test_invalid_url_name_template_type_keeps_default(self) -> None:
        """Non-str ``URL_NAME_TEMPLATE`` is ignored."""
        with override_settings(NEXT_FRAMEWORK={"URL_NAME_TEMPLATE": 123}):  # type: ignore[dict-item]
            next_framework_settings.reload()
            assert (
                NextFrameworkSettings.DEFAULTS["URL_NAME_TEMPLATE"]
                == next_framework_settings.URL_NAME_TEMPLATE
            )


class TestNextFrameworkSettingsFlatMerge:
    """Unit tests for :meth:`NextFrameworkSettings._build_flat_merged`."""

    @pytest.mark.parametrize(
        ("user", "expected_routers_len"),
        [
            (None, 1),
            ({}, 1),
            ({"DEFAULT_PAGE_ROUTERS": []}, 0),
        ],
        ids=["none_user", "empty_user_dict", "explicit_empty_routers"],
    )
    def test_build_flat_merged_routers_length(
        self,
        fresh_next_framework_settings: NextFrameworkSettings,
        user: dict[str, Any] | None,
        expected_routers_len: int,
    ) -> None:
        """``DEFAULT_PAGE_ROUTERS`` length follows the merged user value."""
        merged = fresh_next_framework_settings._build_flat_merged(user)
        assert len(merged["DEFAULT_PAGE_ROUTERS"]) == expected_routers_len

    def test_build_flat_merge_empty_component_backends(
        self,
        fresh_next_framework_settings: NextFrameworkSettings,
    ) -> None:
        """Explicit empty ``DEFAULT_COMPONENT_BACKENDS`` is preserved."""
        user = {"DEFAULT_COMPONENT_BACKENDS": []}
        merged = fresh_next_framework_settings._build_flat_merged(user)
        assert merged["DEFAULT_COMPONENT_BACKENDS"] == []


class TestFlatNextFrameworkBehavior:
    """Per-key overrides without nested ``PAGES`` / ``COMPONENTS`` namespaces."""

    def test_cannot_assign_framework_keys(self) -> None:
        """Top-level keys must not be assigned on the settings object."""
        with pytest.raises(AttributeError, match="cannot be assigned"):
            next_framework_settings.URL_NAME_TEMPLATE = "x"  # type: ignore[misc]

    def test_perform_import_raises_import_error(self) -> None:
        """Invalid dotted path raises ``ImportError`` with context."""
        with pytest.raises(ImportError, match="no_such_module_zzz"):
            perform_import("no_such_module_zzz.ClassName", "TEST_SETTING")

    def test_perform_import_returns_non_string_unchanged(self) -> None:
        """Non-string values are returned as-is (for future ``IMPORT_STRINGS`` use)."""
        assert perform_import(42, "X") == 42

    def test_setattr_allows_internal_attributes(self) -> None:
        """Attributes outside ``DEFAULTS`` keys may be set (e.g. tests, hooks)."""
        try:
            next_framework_settings._coverage_probe = 1
            assert next_framework_settings._coverage_probe == 1
        finally:
            del next_framework_settings._coverage_probe
