...
class TestInvalidEnvelopeAddressesOnlyTheFailedForm:
    ...
    def test_only_the_failed_form_is_a_target(self, client: NextClient) -> None:
        response = client.post_action(
            "rename_board_form", {"title": ""}, origin="/board_forms/", partial=True
        )
        envelope = envelope_of(response)
        assert envelope.op_verbs() == ["morph"]
        assert envelope.form_targets() == [_uid("rename_board_form")]
    ...
    def test_neighbouring_form_uids_appear_in_no_target(
        ...
        column_uid = _uid("create_column_form")
        archive_uid = _uid("archive_board_form")
        assert all(target != {"form": column_uid} for target in targets)
        assert all(target != {"form": archive_uid} for target in targets)

    def test_the_only_op_carries_the_extract_flag(self, client: NextClient) -> None:
        response = client.post_action(
            "rename_board_form", {"title": ""}, origin="/board_forms/", partial=True
        )
...
class TestInvalidEnvelopeHeaderContract:
    ...
    def test_status_content_type_and_form_headers(self, client: NextClient) -> None:
        response = client.post_action(
            "rename_board_form", {"title": ""}, origin="/board_forms/", partial=True
        )
        ...
        assert response["X-Next-Action"] == _uid("rename_board_form")


class TestInvalidFormMetaIsMachineReadable:
    """The form meta carries machine-readable errors from the field specs."""

    def test_meta_reports_invalid_with_field_errors(self, client: NextClient) -> None:
        response = client.post_action(
            "rename_board_form", {"title": ""}, origin="/board_forms/", partial=True
        )
        meta = envelope_of(response).form_meta()
        assert meta == {
            "uid": _uid("rename_board_form"),
            "valid": False,
...
class TestInvalidWithZoneMorphsTheZone:
    ...
    def test_bound_form_errors_reach_the_zone_body(self, client: NextClient) -> None:
        response = client.post_action(
            "zoned_rename_form",
            {"title": ""},
            origin="/board_settings/",
    ...
    def test_zone_morph_still_carries_form_meta(self, client: NextClient) -> None:
        response = client.post_action(
            "zoned_rename_form",
            {"title": ""},
            origin="/board_settings/",
...
class TestResultAuthoredPatchPassesThrough:
    ...
    def test_authored_envelope_is_served_verbatim(self, client: NextClient) -> None:
        response = client.post_action(
...
class TestResultRichResponseFallsThrough:
    ...
    def test_plain_html_response_is_not_an_envelope(self, client: NextClient) -> None:
        response = client.post_action(
...
class TestSuccessFunnelMorphsAndDrainsMessages:
    ...
    def test_zone_morph_and_message_toasts(self, client: NextClient) -> None:
        response = client.post_action(
            "zoned_rename_form",
            {"title": "Renamed"},
            origin="/board_settings/",
    ...
    def test_success_omits_the_invalid_headers(self, client: NextClient) -> None:
        response = client.post_action(
            "zoned_rename_form",
            {"title": "Renamed"},
            origin="/board_settings/",
...
class TestSuccessFunnelExtractMorphWithoutZone:
    ...
    def test_extract_morph_of_the_form_by_uid(self, client: NextClient) -> None:
        response = client.post_action(
            "regression_form", {"name": "ok"}, origin="/", partial=True
        )
        envelope = envelope_of(response)
        assert envelope.op_verbs() == ["morph"]
        assert envelope.form_targets() == [_uid("regression_form")]
