...
from next.partial.headers import CONTENT_TYPE, REQUEST_FLAG
from next.partial.patches import UnknownContextNameError, UnknownPatchOpError
...
def _partial_request(origin: str = "/zoned/"):
    """Return a partial POST whose origin field resolves to a real page."""
    return RequestFactory().post(
        "/_next/form/x/", data={ORIGIN_FIELD_NAME: origin}, **_PARTIAL_META
    )


def _plain_request(origin: str = "/zoned/"):
    """Return a non-partial POST whose origin field resolves to a real page."""
    return RequestFactory().post("/_next/form/x/", data={ORIGIN_FIELD_NAME: origin})
...
class TestMorphZone:
    ...
    def test_zone_body_morphs_in_place(self) -> None:
        envelope = Patches(_partial_request()).morph(zone="alpha").envelope()
...
class TestMorphComponent:
    ...
    def test_component_renders_with_props(self) -> None:
        ...
        with (
            patch("next.components.get_component", return_value=info),
            patch(
                "next.components.render_component", return_value="<aside>card</aside>"
            ) as render,
        ):
            envelope = (
                Patches(_partial_request())
    ...
    def test_invisible_component_raises(self) -> None:
        with (
            patch("next.components.get_component", return_value=None),
            pytest.raises(LookupError),
        ):
            Patches(_partial_request()).morph(component="ghost")
...
class TestStandaloneVerbs:
    ...
    def test_push_url_validates_same_site(self) -> None:
        envelope = Patches(_partial_request()).push_url("/next/").envelope()
        assert envelope.ops[0].as_dict() == {
            "op": "url",
            "action": "push",
            "href": "/next/",
    ...
    def test_redirect_internal_is_validated(self) -> None:
        envelope = Patches(_partial_request()).redirect("/safe/").envelope()
...
class TestContextPatch:
    ...
    def test_registered_serialize_name_serialises(self) -> None:
        envelope = Patches(_partial_request()).context(flag=True).envelope()
        assert envelope.ops[0].as_dict() == {
            "op": "context",
            "data": {"flag": True},
        }
    ...
    def test_unregistered_name_raises_unknown_context(self) -> None:
        with pytest.raises(UnknownContextNameError) as exc:
            Patches(_partial_request()).context(secret="leak")
